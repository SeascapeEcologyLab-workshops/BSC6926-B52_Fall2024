...
*Short answer: The year with the highest mean CPUE is `{r} max(df_m$CPUE_m)` in `{r} df_m$Year[which.max(df_m$CPUE_m)]`. The year with the lowest mean CPUE is `{r} min(df_m$CPUE_m)` in `{r} df_m$Year[which.min(df_m$CPUE_m)]`. The species with the highest CPUE is always Spotted seatrout, but the minimum is both Crevalle Jack and Red drum depending on the year*
